...
const GeneralSection = () => {
  ...
  useEffect(() => {
    const fetchBusinessData = async () => {
      try {
        if (id) {
          const response = await axios.get(`https://crm.qrps.ca:5000/api/business/${id}`);
    ...
    const fetchUserData = async () => {
      try {
        const userResponse = await axios.get(`https://crm.qrps.ca:5000/api/user/${userData.userId}`);
        ...
        const businessesResponse = await axios.get(`https://crm.qrps.ca:5000/api/business/user/${userData.userId}`);
        setUserData(prev => ({
          ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        await axios.put(`https://crm.qrps.ca:5000/api/business/${id}`, formData);
      } else {
        await axios.post('https://crm.qrps.ca:5000/api/business', {
          ...formData,
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this business?')) {
      try {
        await axios.delete(`https://crm.qrps.ca:5000/api/business/${id}`);
        navigate(`/mybusinesses`);
      } catch (error) {
  ...
  const handleBannerDownload = async () => {
    ...
    try {
      const qrCodeDataURL = await QRCodeLib.toDataURL(`https://crm.qrps.ca/form/en/${id}`, { width: qrCodeSize });
  ...
  const businessFormUrl = `https://crm.qrps.ca/form/en/${id}`;
  return (
